...
public class GetWeatherTenDaysTest extends AbstractTest {
    ...
    void get_shouldReturn500() throws IOException {
        ...
        logger.debug("Формирование мока для GET /forecasts/v1/daily/20day/294021");
        stubFor(get(urlPathEqualTo("/forecasts/v1/daily/20day/294021"))
                .willReturn(aResponse()
                        .withStatus(500).withBody("ERROR")));

        CloseableHttpClient httpClient = HttpClients.createDefault();
        HttpGet request = new HttpGet(getBaseUrl() + "/forecasts/v1/daily/20day/294021");
        logger.debug("http клиент создан");

        HttpResponse response = httpClient.execute(request);

        verify(getRequestedFor(urlPathEqualTo("/forecasts/v1/daily/20day/294021")));
